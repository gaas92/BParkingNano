...
class TrackMerger_h : public edm::global::EDProducer<> {


public:

  //would it be useful to give this a bit more standard structure?
  explicit TrackMerger_h(const edm::ParameterSet &cfg):
...
void TrackMerger_h::produce(edm::StreamID, edm::Event &evt, edm::EventSetup const &stp) const {
  ...
  // HCL Tracks y lost Tracks son un objeto PackedCandidate --- candidates reconstructed by the ParticleFlow algorithm
  // HCL packedPFCandidates 
  // HCL lostTracks
  ...
  */
  ...
  for( unsigned int iTrk=0; iTrk<totalTracks; ++iTrk ) {
    // Solo se esta eligiendo el track de la coleccion adecuada de acuerdo a su indice
    ...
    // HCL Se itera sobre todos los triggering muons en busqueda de alguno que no satisfaga
    // HCL los cortes dados por el usuario en dr y dz respecto al track
    // HCL Se ignora al track que no satisfaga estas condiciones al menos para un trg muon
    ...
    pcand.addUserFloat("px", trk.px());
    pcand.addUserFloat("py", trk.py());
    pcand.addUserFloat("pz", trk.pz());
    pcand.addUserFloat("mass", trk.mass());
    pcand.addUserFloat("energy", trk.energy()); 
...
DEFINE_FWK_MODULE(TrackMerger_h);